...
const PaymentAOC = () => {
  ...
  // Pixel tracking helper function
  const trackEvent = (eventName, eventData = {}) => {
    try {
      if (window.fbq) {
        window.fbq('track', eventName, eventData);
      } else {
        console.warn(`Meta Pixel event ${eventName} could not be tracked`);
      }
    } catch (error) {
  ...
  // Add useEffect to initialize Meta Pixel
  useEffect(() => {
    // Create global fbq function if not exists
    ...
    // Noscript tag for no-JavaScript tracking
    ...
    trackEvent('InitiateCheckout', {
      value: 89.00,
      currency: 'INR',
      content_category: 'Art of Conversation'
    });
  }, []);
