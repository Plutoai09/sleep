...
useEffect(() => {
  if (audioSrc) {
    ...
    // Set audio properties for optimal loading
    audio.preload = "auto";
    ...
    const configureAudio = () => {
      if (audio.mozAutoplayEnabled === undefined) {
        audio.bufferSize = 16384;
      } else {
    ...
    return () => {
      ...
      audio.removeEventListener('canplaythrough', () => setIsBuffering(false));
